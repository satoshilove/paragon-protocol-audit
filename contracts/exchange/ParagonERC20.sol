...
contract ParagonERC20 {
    ...
    function _mint(address to, uint256 value) internal {
        totalSupply += value;                    // No SafeMath needed in 0.8.0+
        balanceOf[to] += value;                  // Built-in overflow protection
        emit Transfer(address(0), to, value);
    }

    function _burn(address from, uint256 value) internal {
        balanceOf[from] -= value;                // No SafeMath needed in 0.8.0+
        totalSupply -= value;                    // Built-in underflow protection
    ...
    function transferFrom(address from, address to, uint256 value) external returns (bool) {
        if (allowance[from][msg.sender] != type(uint256).max) {  // Updated from uint(-1)
            allowance[from][msg.sender] -= value;                // No SafeMath needed
        }
