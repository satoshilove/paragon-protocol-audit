...
describe("P10 dust outside vault", function () {
  async function deployFixture() {
    const [owner, user, attacker, feeRecipient] = await ethers.getSigners();
    ...
    await (await tokenA.mint(attacker.address, E("10"))).wait();
    await (await tokenB.mint(attacker.address, E("10"))).wait();
    await (await tokenC.mint(attacker.address, 10_000_000)).wait();
  ...
  async function mintBasketSeed(manager, user) {
    await (await manager.connect(user).mintBasketExact([E("0.5"), E("1"), 1_000_000], user.address)).wait();
  }

  it("dust sent to execution manager does not break single-token mint", async function () {
    const { attacker, user, tokenA, tokenB, exec, manager } = await deployFixture();

    await (await tokenA.connect(attacker).transfer(await exec.getAddress(), E("0.0001"))).wait();
  ...
  it("dust sent to venue does not break single-token mint", async function () {
    const { attacker, user, tokenA, tokenB, venue, manager } = await deployFixture();

    await (await tokenA.connect(attacker).transfer(await venue.getAddress(), E("0.0001"))).wait();
  ...
  it("dust sent to adapter does not break single-token mint", async function () {
    const { attacker, user, tokenA, tokenB, adapter, manager } = await deployFixture();

    await (await tokenA.connect(attacker).transfer(await adapter.getAddress(), E("0.0001"))).wait();
  ...
  it("dust sent to execution manager does not break redeemSingle into underlying", async function () {
    const { attacker, user, tokenB, exec, manager, tokenA } = await deployFixture();

    await mintBasketSeed(manager, user);
    await (await tokenB.connect(attacker).transfer(await exec.getAddress(), E("0.0001"))).wait();
