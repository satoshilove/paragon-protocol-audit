...
describe("P10 preview consistency", function () {
  async function deployFixture() {
    ...
    return { user, tokenA, tokenB, tokenC, p10, manager };
  }
  ...
  it("previewMintSingle is a conservative estimate versus realized mint", async function () {
    const { user, tokenB, p10, manager } = await deployFixture();

    const amountIn = E("2");
    const preview = await manager.previewMintSingle(await tokenB.getAddress(), amountIn);
    ...
    const before = await p10.balanceOf(user.address);

    await (await manager.connect(user).mintSingle(
    ...
    const afterBal = await p10.balanceOf(user.address);
    const realized = afterBal - before;

    expect(realized).to.be.gte((preview * 98n) / 100n);
  });

  it("previewRedeemSingle is a conservative estimate versus realized redeem", async function () {
    const { user, tokenA, manager } = await deployFixture();

    await (await manager.connect(user).mintBasketExact([E("0.5"), E("1"), 1_000_000], user.address)).wait();

    const p10In = E("0.5");
    const preview = await manager.previewRedeemSingle(p10In, await tokenA.getAddress());
    ...
    await (await manager.connect(user).redeemSingle(
      p10In,
    ...
    const afterBal = await tokenA.balanceOf(user.address);
    const realized = afterBal - before;
